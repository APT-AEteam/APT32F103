...
///ALRA/ALRB
/*
#define RTC_ALMA_POS		(0)
#define RTC_ALMB_POS		(1)
#define RTC_ALM_WDSEL_POS	(24ul)
//#define RTC_ALM_DAY_MSK	(0x3f << RTC_ALM_DAY_POS)
#define RTC_ALM_WDSEL_MSK	(0x7f << RTC_ALM_DAY_POS)
...
//#define Sundays    (0x1 << 24)
//#define Saturdays  (0x1 << 25)
//#define Fridays    (0x1 << 26)
//#define Thursdays  (0x1 << 27)
//#define Wednesdays (0x1 << 28)
//#define Tuesdays   (0x1 << 29)
//#define Mondays    (0x1 << 30)

//#define EveryDay   (0x0)
//#define Sundays    (0x1)
//#define Saturdays  (0x2)
//#define Fridays    (0x4)
//#define Thursdays  (0x8)
//#define Wednesdays (0x10)
//#define Tuesdays   (0x20)
//#define Mondays    (0x40)
...
//#define RTC_ALMB_POS		(1)
#define RTC_ALM_WDSEL_POS	(24ul)
#define RTC_ALM_WDSEL_MSK	(0x7f << RTC_ALM_WDSEL_POS)
...
#define RTC_ALM_MODE 	(0x80808080)
...
#define RTC_TRGEV1PRD_POS	(4)
#define RTC_TRGEV1PRD_MSK	(0xf << RTC_TRGEV1PRD_POS)
#define RTC_TRGEV0CNT_POS	(16)
#define RTC_TRGEV0CNT_MSK	(0xf << RTC_TRGEV0CNT_POS)
#define RTC_TRGEV1CNT_POS	(20)
#define RTC_TRGEV1CNT_MSK	(0xf << RTC_TRGEV0CNT_POS)
//EVSWF
...
static inline uint8_t csp_rtc_get_clksrc(csp_rtc_t *ptRtcBase)
{
	return (uint8_t)((ptRtcBase->CCR  & RTC_CLKSRC_MSK) >> 24);
}

static inline void csp_rtc_set_time_hour(csp_rtc_t *ptRtcBase, rtc_ampm_e ePm, uint8_t byVal)
{
	ptRtcBase->TIMR = (ptRtcBase->TIMR  & (~RTC_HOR_MSK) & (~RTC_PM_MSK)) |(byVal << RTC_HORU_POS) | (ePm << RTC_PM_POS);
}
...
static inline void csp_rtc_alm_enable(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm, bool bEnable)
{

	ptRtcBase->KEY = 0xCA53;
	if (bEnable)
		ptRtcBase->CR |= 0x1 << (3 + eAlm);
	else	
		ptRtcBase->CR &= ~(0x1 << (3 + eAlm));
	ptRtcBase->KEY = 0x0;
	while(ptRtcBase->CR & RTC_BSY);
...
static inline void csp_rtc_alm_set_wday(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm,  uint8_t byVal)
{
	if (eAlm == RTC_ALMA_POS)
		ptRtcBase->ALRA = (ptRtcBase->ALRA & (~RTC_ALM_WDSEL_MSK)) |   (byVal << RTC_ALM_WDSEL_POS);
	else
		ptRtcBase->ALRB = (ptRtcBase->ALRB & (~RTC_ALM_WDSEL_MSK)) |   (byVal << RTC_ALM_WDSEL_POS);
}

static inline void csp_rtc_alm_set_hour(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm, rtc_ampm_e ePm, uint8_t byVal)
{
	if (eAlm == RTC_ALMA_POS)
		ptRtcBase->ALRA = (ptRtcBase->ALRA & (~RTC_ALM_HOR_MSK) & (~RTC_ALM_PM)) |  (byVal << RTC_ALM_HORU_POS) | (ePm<<22);
	else
		ptRtcBase->ALRB = (ptRtcBase->ALRB & (~RTC_ALM_HOR_MSK) & (~RTC_ALM_PM)) |  (byVal << RTC_ALM_HORU_POS) | (ePm<<22);
}

static inline void csp_rtc_alm_set_min(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm, uint8_t byVal)
{
	if (eAlm == RTC_ALMA_POS)
...
static inline void csp_rtc_alm_set_sec(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm,  uint8_t byVal)
{
	if (eAlm == RTC_ALMA_POS)
...
//static inline uint8_t csp_rtc_alm_read_mday(csp_rtc_t *ptRtcBase, uint8_t eAlm)
//{
//	if (eAlm == RTC_ALMA_POS)
...
static inline uint8_t csp_rtc_alm_read_wday(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm)
{
	if (eAlm == RTC_ALMA_POS)
...
static inline uint8_t csp_rtc_alm_read_hour(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm)
{
	if (eAlm == RTC_ALMA_POS)
...
static inline uint8_t csp_rtc_alm_read_min(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm)
{
	if (eAlm == RTC_ALMA_POS)
...
static inline uint8_t csp_rtc_alm_read_sec(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm)
{
	if (eAlm == RTC_ALMA_POS)
...
//static inline bool csp_rtc_alm_read_dmsk(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm)
//{
//	if (eAlm == RTC_ALMA_POS)
...
static inline bool csp_rtc_alm_read_wdsel(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm)
{
	if (eAlm == RTC_ALMA_POS)
		return ( (ptRtcBase->ALRA & RTC_ALM_WDSEL_MSK) >> RTC_ALM_WDSEL_POS);
	else
...
static inline void csp_rtc_alm_set_mode(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm, bool bHmsk, bool bMmsk, bool bSmsk)
{
	ptRtcBase->KEY = 0xCA53;
	if (eAlm == RTC_ALMA_POS)
//		ptRtcBase->ALRA = (ptRtcBase->ALRA & (~RTC_ALM_MODE)) | bWdsel << RTC_ALM_WDSEL_POS |(bDmsk << RTC_ALM_DMSK_POS) | (bHmsk << RTC_ALM_HMSK_POS) | (bSmsk << RTC_ALM_SMSK_POS);
		ptRtcBase->ALRA = (ptRtcBase->ALRA & (~RTC_ALM_MODE))  | (bHmsk << RTC_ALM_HMSK_POS) | (bMmsk << RTC_ALM_MMSK_POS)| (bSmsk << RTC_ALM_SMSK_POS);
	else
//		ptRtcBase->ALRB = (ptRtcBase->ALRB & (~RTC_ALM_MODE)) | (bDmsk << RTC_ALM_DMSK_POS) | (bHmsk << RTC_ALM_HMSK_POS) | (bSmsk << RTC_ALM_SMSK_POS);
		ptRtcBase->ALRB = (ptRtcBase->ALRB & (~RTC_ALM_MODE))  | (bHmsk << RTC_ALM_HMSK_POS) | (bMmsk << RTC_ALM_MMSK_POS)| (bSmsk << RTC_ALM_SMSK_POS);
	ptRtcBase->KEY = 0x0;
}
...
csp_error_t csp_rtc_set_time(csp_rtc_t *ptRtcBase, rtc_ampm_e ePm, uint8_t byHor, uint8_t byMin,uint8_t bySec);
//csp_error_t csp_rtc_alm_set(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm, uint8_t byDay, uint8_t ePm, uint8_t byHor, uint8_t byMin,uint8_t bySec);
void csp_rtc_alm_reset(csp_rtc_t *ptRtcBase, rtc_alarm_e eAlm);
csp_error_t csp_rtc_set_trgsrc(csp_rtc_t *ptRtcBase, uint8_t byTrg, csp_rtc_trgsrc_e eSrc);
