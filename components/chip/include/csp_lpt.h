...
typedef struct
{
   ...
   __IM  uint32_t  SR;        	//0x0044    Synchronous reg
} csp_lpt_t;
...
static inline void csp_lpt_data_update(csp_lpt_t *ptLptBase,uint16_t hwPrdr, uint16_t hwCmp)
{
	...
	while(ptLptBase->SR&LPT_PRDR_BUSY);
	ptLptBase->CMP = hwCmp;		
	while(ptLptBase->SR&LPT_CMP_BUSY);
}
...
{
	ptLptBase->CNT = hwCnt;
	while(ptLptBase->SR&LPT_CNT_BUSY);
}
...
static inline void csp_lpt_set_clk(csp_lpt_t *ptLptBase, lpt_css_e eClk, lpt_pscdiv_e ePscDiv)
{
	...
	while(ptLptBase->SR&LPT_PSCR_BUSY);
...
static inline void csp_lpt_set_filter(csp_lpt_t *ptLptBase, uint8_t byFreDiv, lpt_fltdeb_e byNTimes)
{
	ptLptBase->CEDR = (ptLptBase->CEDR & ~(LPT_FLTCKPRS_MSK)) | LPT_FLTCKPRS(byFreDiv);
	while(ptLptBase->SR&LPT_CEDR_BUSY);
	ptLptBase->CR = (ptLptBase->CR & ~(LPT_FLTDEB_MSK)) | (byNTimes << LPT_FLTDEB_POS);
	while(ptLptBase->SR&LPT_CR_BUSY);
}
